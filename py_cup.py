...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeVideoContent(self):
		rsp = self.fetch("https://cupfox.app/")
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		suffix = '-'.join(params)
		url = 'https://cupfox.app//vodshow/{0}.html'.format(suffix)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
		aList = root.xpath("//div[@class='stui-vodlist__box']/a")
		videos = []
	...
	def detailContent(self,array):
		tid = array[0]
		url = 'https://cupfox.app//spx/{0}.html'.format(tid)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
	...
	def searchContent(self,key,quick):		
		url = 'https://cupfox.app//index.php/ajax/suggest?mid=1&wd={0}'.format(key)
		# getHeader()
		rsp = self.fetch(url)
	...
	def playerContent(self,flag,id,vipFlags):
		result = {}
		url = 'https://cupfox.app/sp/{0}.html'.format(id)
		rsp = self.fetch(url)
		root = self.html(rsp.text)
		...
		scriptUrl = 'https://cupfox.app/static/js/playerconfig.js'
